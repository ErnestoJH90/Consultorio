 ...
 * @version $Date: 2019/01/31 08:48:48 $
 */
...
class Calendar
{
	...
	private function __construct($year) {
		...
		$this->pdf->setAutoPageBreak(false, 0.0);
	...
	private function generateMonthPage($month){
		...
		$p->setFont($this->font, '', 40.0);
		$p->setTextColor(0, 0, 0);
		$w = $p->widthOf($us);
		$p->setXY((210.0 - $w)/2, $y); // center month name on A4
		$p->write(0.0, $us);
		...
		for($g = 1; $g <= $gg; $g++){
			...
			if( $day_week == 6 || $this->isHoliday($month, $g) )
				...
				$p->setTextColor(0, 0, 0);
			$p->setFont($this->font, $style, 18.0);
			$p->write(0.0, UString::fromUTF8($s));
