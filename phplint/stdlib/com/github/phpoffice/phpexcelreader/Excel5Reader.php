 ...
 * @version    $Date: 2019/01/31 12:22:04 $
 */
...
//		$this->readFilter = new DefaultReadFilter();
	...
	private static function getInt4d($data, $pos) {
		...
		if ($_or_24 >= 128) {
			...
			$_ord_24 = ((int) (-abs(256.0 - $_or_24))) << 24;
		} else {
...
//	}
	...
	private static function extractNumber($data) {
		...
		$value = $mantissa / (float)pow(2.0, (20.0 - $exp));

		if ($mantissalow1 != 0) {
			$value += 1 / (float)pow(2.0, (21.0 - $exp));
		}

		$value += $mantissalow2 / (float)pow(2.0, (52.0 - $exp));
		if ($sign != 0) {
			$value *= -1;
		}
	...
	private static function getIEEE754($rknum) {
		if (($rknum & 0x02) != 0) {
			...
			$value = $mantissa / (float)pow(2.0, (20.0 - ($exp - 1023)));
			if ($sign != 0) {
				$value = -1 * $value;
			}
