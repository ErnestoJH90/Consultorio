 ...
 * @version $Date: 2018/09/14 19:48:42 $
 */
class TypeDescriptor {
	...
	private function parseName(){
		...
		// Try fast look-up:
		if( isset(self::$predefined[$name]) )
			return self::$predefined[$name];
		
		// Check mispelled case letters:
		$name_low = strtolower($name);
		if( isset(self::$predefined[$name_low]) ){
		...
		if( Scanner::$ascii_ext_check && preg_match(self::$ascii_name_pattern, $name) !== 1 )
				throw new TypeDescriptorException("invalid characters in identifier: " . Strings::toLiteral($name));
