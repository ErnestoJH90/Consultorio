...
class ClassType extends Type implements Sortable, Hashable {
	 ...
	 * @var GenericHashMap<CaseInsensitiveString,ClassMethod>
	 */
	public $methods;
...
.*/
	...
	public function __construct($name, $where, $is_internal){
		...
		$this->methods = new GenericHashMap/*.<CaseInsensitiveString,ClassMethod>.*/();
	}
	...
	/**
	 * Initializes a new, empty actualized classes cache. It should be called
	 * by the global module before using this class.
	 */
...
//		$actual_class->parent_destructor_called = ignored; checked on template only
		...
		// Actualize methods:
		$actual_methods = new GenericHashMap/*.<CaseInsensitiveString,ClassMethod>.*/();
		foreach($actual_class->methods as $method){
			if( $method->is_static ){
