 ...
 * @version $Date: 2019/01/29 11:35:10 $
 */
class FormalArguments {
	...
	private static function parseArg($globals, $dbw, $opt_arg)
	{
		...
		$a->is_nullable = TypeDecl::$is_nullable;
		$a->is_php_code = TypeDecl::$is_php_code;
		if( $a->type === Globals::$void_type ){
			$globals->logger->error($scanner->here(),
			"invalid `void' type for formal argument");
...
//				$db->where->error("missing parameter \$" . $a->name . " in DocBlock");
		...
		if ($a->type === NULL) {
			$globals->logger->error($scanner->here(),
			"undefined type for argument $" . $a->name . ". Hint: you may:"
			. "\n- Add DocBlock line-tag, example: @param int $" . $a->name
			. "\n- Use type-hint,         example: int $" . $a->name
			. "\n- Use meta-code,         example: /*.int.*/ $" . $a->name
			. "\n- Assign default value,  example: $" . $a->name . " = 123"
			);
			$a->type = Globals::$unknown_type;
		}
		...
		$v->assigned = $v->assigned_once =
			! $a->reference
			|| $a->reference && ! $a->reference_return;
	...
	public static function parse($globals, $dbw, $sign) {
		...
		if( $scanner->sym === Symbol::$sym_colon ){
			...
			if( $t === NULL ){
				...
				$sign->is_nullable = TypeDecl::$is_nullable;
				$sign->is_php_code = TypeDecl::$is_php_code;
				$sign->returns = $t;
			}
		}
	}
