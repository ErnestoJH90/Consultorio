 ...
 * @version $Date: 2018/12/22 23:38:03 $
 */
 ...
 */
	 ...
	 * entry box. This event is captured by putting an invisible button just
	...
	/**
	 * Event capturing user's navigation in the browser's history, typically a
	 * "back". This implementation invokes the defaultEvent() method, but client
	 * code may re-implement it in an attempt to meet user's expectation to leave
	 * the page in some way, for example performing the same action of the "Cancel",
	 * "Dismiss" or "Stop" buttons.
	 */
	...
	/**
	 * Event capturing user's reload action on the browser's navigation bar.
	 * This implementation invokes the defaultEvent() method, but client code may
	 * re-implement it in an attempt to meet user's expectation to see updated
	 * data displayed.
	 */
	...
	/**
	 * Parses the handler method and returns the panel and the method. For
	...
	{
		// Create an anchor that clicks on an invisible button that performs
		// the POST.
	 ...
	 * should contain exactly one invocation of this method and another for the
	...
	function open()
	{
		...
		// Capture the "implicit submission mechanism", i.e. the "enter" key:
		$this->addAttributes("id=phplint_defaultButton style='display: none'");
		$this->button("", "defaultButton");
		
		// An attempt to capture the "back" button event.
		UserSession::setDefaultCallForward();
		$b = /*. (mixed[int]) .*/ array();
		$b[0] = FALSE; // not a postback, form fields not available
		$b[1] = "*";
		$b[2] = "browserBackEvent";
		// UserSession will use the following link as the default -- no need to
		// really put it on the generated page:
		try {
			/* $url = */ UserSession::link(UserSession::class . "::invokeCallBackward", $b);
		} catch (ErrorException $e) {
			throw new RuntimeException($e->getMessage(), 1, $e);
		}
		
		// Replacing our entry in the browser's history prevents the browser from
		// displaying the boring "Are you sure to re-submit the form data?" as the
		// user clicks "Reload", and allows us to precisely detect this event
		// happened:
		$b = /*. (mixed[int]) .*/ array();
		$b[0] = FALSE; // not a postback, form fields not available
		$b[1] = "*";
		$b[2] = "browserReloadEvent";
		try {
			$url = UserSession::link(UserSession::class . "::invokeCallBackward", $b);
		} catch (ErrorException $e) {
			throw new RuntimeException($e->getMessage(), 1, $e);
	 ...
	 * retrieved (this may happen for a call-backward invocation from bt stack
