 ...
 * @version $Date: 2018/12/30 05:26:10 $
 */
class UserSession {
	 ...
	 * "i" parameter from the request is invalid, typically because the user
	 * navigated the history of the browser or clicked "back", "reload" or
	 * "forward" in the navigation bar.
	 * @access private
	 */
	...
	/**
	 * If the cookie has to be sent and accepted only over a secure HTTPS
	 * connection.
	 * @var boolean
	 */
	 ...
	 * Anchors and form actions will use this URL. The dispatcher page should
	 * simply instantiate this object, which in turn will renew the session
	 ...
	 * the default value is returned instead, otherwise it is an exception.
	 * @param string $name Name of the parameter.
	 * @param string $def Default value returned if the parameter is missing.
	 * @return string Value of the parameter.
	 * @throws RuntimeException Parameter is missing and no default value set.
	 ...
	 * triggers the invocation of the specified call-forward function.
	 * Inside a currently open form, performs a postback of the form data
		...
		try {
			...
			if( ! self::$us->form_open ){
				...
				echo "<a href='#' ", $saved_add_attributes,
					" onclick=\"document.getElementById('$button_id').click(); return false;\">$text_html</a>";
			}
	 ...
	 * triggers the invocation of a call-forward function.
	 ...
	 */
	private static function isValidSessionValueFormat($x)
	{
	...
	private function isSessionAvailable()
	{
		...
		if( ! self::isValidSessionValueFormat($cookie_value) )
			return FALSE;
	...
	private function staleSessionsCleanup()
	{
		...
		do {
			...
			if ($entry === FALSE)
				break;
			if( self::isValidSessionValueFormat($entry) ) {
				...
				self::deleteDirectory($session_dir);
			}
		} while (TRUE);
	...
	/**
	 * Delete user's session. If a session was already set, delete the session
	 * and sends the user to the login page. The browser's cookie is always
	 * deleted.
	 * @return void
	 */
	static function logout()
	{
		if( self::$us !== NULL ){
			...
			// Remove any file lock on the current window session:
			self::$us->window_session->close();
			
			// Delete user's session directory:
			try {
				self::deleteDirectory(self::$us->user_session_dir);
			}
			catch(ErrorException $e){
	 ...
	 * @param boolean $secure If the session cookie has to be accepted and sent
	 ...
	 * @throws Exception
	 */
	function __construct($login_name, $base_dir, $dispatcher_page, $login_page, $dashboard_function, $secure)
	{
		$method = $_SERVER['REQUEST_METHOD'];
		...
		if( $this->secure && ! isset($_SERVER["HTTPS"]) ){
			Log::notice("rejecting request from insecure non-HTTPS channel - logout");
			if( ! $this->isSessionAvailable() )
				self::$us = NULL; // reset incomplete instance
			self::logout();
			return;
		}
