...
class FileDownload {
	...
	static function sendFile($path)
	{
		...
		/*
		 * Send the file contents. Note that readfile() alone DOES NOT WORK with
		 * large files because it relies on fpassthru() which in turn tries to
		 * "mmap" the whole file on disk into the address space of the process,
		 * see https://bugs.php.net/bug.php?id=61636
		 * The proposed trickery:
		 * 
		 * while( ob_get_level() )
		 *     ob_end_flush();
		 * readfile($fn);
		 * 
		 * works, but there is a quite large delay before the server starts to
		 * actually send something to the client (mmap issue?), it results even
		 * slower than fopen/fread/fclose in my tests, and sometimes it does not
		 * work at all for unknown reason (wtf!?).
		 * This plain, simpler implementation works well in any situation:
		 */
		$f = fopen($path, "rb");
		while(TRUE){
			if( feof($f) )
